...
st.set_page_config(
    page_title="Ringkasan | Client Ledger",
    page_icon="◆",
    layout="wide",
...
st.sidebar.markdown("---")
st.sidebar.caption("ANALYTICS · EXECUTIVE OVERVIEW")
...
render_hero(
    "Executive Overview",
    "Pantau nilai pelanggan, distribusi segmen, dan area risiko dalam satu tampilan analitik yang siap digunakan untuk presentasi.",
    "CLIENT LEDGER · EXECUTIVE ANALYTICS",
)
...
if not summary.empty:
    ...
    risk_text = (
        f"Lost/Churned mencakup {risk.iloc[0].Persentase:.1f}% pelanggan."
        if not risk.empty
        else "Lost/Churned tidak termasuk dalam filter aktif."
    )
    st.markdown(
        f"<div class='insight'><b>Executive insight</b><br>{top.Segmen} menjadi kontributor revenue terbesar dengan <b>£{top.TotalRevenue:,.0f}</b>. {risk_text}</div>",
...
for col, (_, row) in zip(card_cols, summary.iterrows()):
    ...
    with col:
        st.markdown(
            f"""
            <div style='background:#0D1118;border:1px solid #202938;border-top:3px solid {color};border-radius:14px;padding:16px;min-height:142px;'>
                <div style='font-size:.68rem;color:#8D98A9;text-transform:uppercase;letter-spacing:.08em;font-weight:700;'>{row.Segmen}</div>
                <div style='font-family:Space Grotesk;font-size:1.45rem;font-weight:700;color:#F4F7FB;margin-top:8px;'>{int(row.Jumlah):,}</div>
                <div style='font-size:.72rem;color:#8D98A9;margin-top:2px;'>{row.Persentase:.1f}% pelanggan</div>
                <div style='font-size:.76rem;color:#CBD5E1;margin-top:11px;'>Revenue <b>£{row.TotalRevenue:,.0f}</b></div>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
fig2 = px.bar(
    summary.sort_values("TotalRevenue"),
    x="TotalRevenue",
    y="Segmen",
    orientation="h",
    color="Segmen",
...
with st.expander("Panduan metrik & metodologi"):
    for term, desc in GLOSSARY:
        st.markdown(
            f"<div class='gloss-item'><b>{term}</b><span>{desc}</span></div>",
            unsafe_allow_html=True,
        )
