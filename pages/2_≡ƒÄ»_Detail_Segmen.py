...
import streamlit as st,pandas as pd,plotly.graph_objects as go,plotly.express as px,yaml
...
df=load();st.sidebar.markdown('### Filter analitik');segments=st.sidebar.multiselect('Segmen',ORDER,ORDER);filtered=df[df.Segmen.isin(segments)]
...
for _,r in normalized.iterrows():
    fig.add_trace(go.Scatterpolar(r=[r.Recency,r.Frequency,r.Monetary,r.Recency],theta=['Recency','Frequency','Monetary','Recency'],fill='toself',name=r.Segmen,line_color=COLOR_MAP.get(r.Segmen),fillcolor=COLOR_MAP.get(r.Segmen),opacity=.22))
...
for seg in ORDER:
    ...
    r=summary[summary.Segmen==seg].iloc[0];c=COLOR_MAP[seg]
    st.markdown(f"<div class='seg-card'><div class='seg-badge' style='--seg-color:{c}'>{INITIAL_MAP[seg]}</div><div style='width:100%'><b>{seg}</b><span class='desc'>{INSIGHTS[seg]}</span><div class='seg-stat'><span>{int(r.Jumlah):,} pelanggan</span><span>{r.Persentase if 'Persentase' in r else (r.Jumlah/summary.Jumlah.sum()*100):.1f}% share</span><span>R {r.Recency:.0f} hari</span><span>F {r.Frequency:.1f}×</span><span>M £{r.Monetary:,.0f}</span><span>Revenue £{r.TotalRevenue:,.0f}</span></div></div></div>",unsafe_allow_html=True)
section('Segment comparison')
st.dataframe(summary[['Segmen','Jumlah','Recency','Frequency','Monetary','TotalRevenue']].style.format({'Jumlah':'{:,.0f}','Recency':'{:,.1f}','Frequency':'{:,.1f}','Monetary':'£{:,.0f}','TotalRevenue':'£{:,.0f}'}),use_container_width=True,hide_index=True)
