...
from style_and_pipeline import inject_style,render_hero,render_brand,render_kpis,ORDER
...
df=load();st.sidebar.markdown('### Filter data');segments=st.sidebar.multiselect('Segmen',ORDER,ORDER);search=st.sidebar.text_input('Cari Customer ID','');freq_min,freq_max=int(df.Frequency.min()),int(df.Frequency.max());freq=st.sidebar.slider('Frequency',freq_min,freq_max,(freq_min,freq_max));mon_min,mon_max=float(df.Monetary.min()),float(df.Monetary.max());mon=st.sidebar.slider('Monetary',mon_min,mon_max,(mon_min,mon_max));filtered=df[df.Segmen.isin(segments)&df.Frequency.between(freq[0],freq[1])&df.Monetary.between(mon[0],mon[1])].copy()
